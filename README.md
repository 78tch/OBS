# OBS Studio 設定
## １．設定（プロファイル・シーンコレクション・Mac特有）
OBSには、設定項目がたくさんあるが、大きく３つに分類できる。  
- 「グローバル設定」：「パソコンやアプリ全体」に対する、切り替えられない設定  
- 「ベーシック設定」：「録画の場合」「配信の場合」など、用途ごとに切替える設定（プロファイル）
- 「シーン」：映像素材などソースの設定。映像は解説したいソフトの画面、Webカメラの画面などから設定、音声はデスクトップ音声、マイクなどから設定。  
なにか設定をするとき、この３つのうち、どれに関する設定なのかを把握し、録画のたびに適宜切り替えて使う。  
## ２．切り替えられない全体的な設定
