...
4. 設定項目の例は、「自動更新の有効・無効」、「アクセシビリティで強調色の変更」などで、設定画面「OBS->Preference」の「一般」「アクセシビリティ」、「ツール->WebSocket サーバー設定」などで設定できる。
...
2. ファイル形式はデフォルトの「mkv」。mp4に変換したい場合、事後に「ファイル->録画の再多重化」で録画ファイルを選択し、「再多重化」とすると、mp4形式に変換される。これを自動的に実行する設定
