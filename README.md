...
3. 設定した内容は「obs-studio -> basic->profiles->プロファイル名->basic.ini」に保存される。テキストファイルなので、読めば、設定内容が分かる。「高画質録画用」「低画質録画用」など、用途ごとに呼び出して切り替えることができるが、用途が「フルHD60fps」のみ、などと決まっていたら、切り替える必要はない。
